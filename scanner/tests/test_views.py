...
class ViewTests(SimpleTestCase):
    ...
    def test_home(self):
        response=self.client.get('/');self.assertEqual(response.status_code,200);self.assertContains(response,'Dutch labels. In English.')
    ...
    def test_no_key_no_fake_success(self):self.assertEqual(self.client.post('/api/scan/').status_code,503)
